...
export default function Contact() {
  ...
  return (
    <section id="contact" ref={ref} className="relative py-16 sm:py-32 lg:min-h-screen lg:flex lg:items-center overflow-hidden">
      {/* Mesh gradient */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="mesh-gradient w-[400px] sm:w-[600px] h-[400px] sm:h-[600px] top-0 left-1/2 -translate-x-1/2"
          style={{ background: "radial-gradient(circle, #FF7A00 0%, transparent 70%)", opacity: 0.08 }}
        />
      </div>

      <div className="relative w-full max-w-5xl mx-auto px-5 sm:px-6">
        {/* Big CTA block */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7 }}
          className="relative rounded-2xl sm:rounded-3xl border border-[#FF7A00]/20 bg-card-bg p-6 sm:p-14 text-center mb-8 sm:mb-10 overflow-hidden"
        >
          {/* Inner glow */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 sm:w-72 h-48 sm:h-72 bg-[#FF7A00]/10 rounded-full blur-3xl pointer-events-none" />

          <div className="relative">
            <span className="inline-block text-[11px] sm:text-sm font-semibold text-[#FF7A00] border border-[#FF7A00]/20 bg-[#FF7A00]/[0.06] px-3 sm:px-4 py-1 sm:py-1.5 rounded-full mb-4 sm:mb-6">
              Ready to Scale?
            </span>
